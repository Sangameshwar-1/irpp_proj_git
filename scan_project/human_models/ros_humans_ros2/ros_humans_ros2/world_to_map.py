...
def _draw_box(grid, ox, oy, res, cx, cy, sx, sy, yaw, inflate):
    """Fill a (possibly rotated) axis-aligned box."""
    H, W = grid.shape
    hx = sx / 2.0 + inflate
    hy = sy / 2.0 + inflate
    ...
    # Inverse rotation matrix (rotate point into box frame)
    ...
    for row in range(row_lo, row_hi + 1):
        ...
        for col in range(col_lo, col_hi + 1):
            ...
            lx_l =  ci * dx - si * dy
            ly_l =  si * dx + ci * dy
            if abs(lx_l) <= hx and abs(ly_l) <= hy:
                grid[row, col] = 0


def _draw_circle(grid, ox, oy, res, cx, cy, radius, inflate):
    """Fill a circle (cylinder / sphere / cone base)."""
    H, W = grid.shape
    r = radius + inflate
    r_pix = r / res + 1
