...
class PointCloudMapper(Node):
    def __init__(self):
        ...
        # Monotonic TF timestamp guard — prevents TF_OLD_DATA warnings
        # when Gazebo sim-time stutters cause out-of-order odom messages.
        self.last_tf_stamp_sec = 0.0
    ...
    def odom_callback(self, msg):
        ...
        #
        # Monotonic guard: only publish if this timestamp is strictly newer
        # than the last one.  When Gazebo sim-time stutters (GUI rendering
        # causes physics to stall), odom messages can arrive out of order,
        # flooding TF with TF_OLD_DATA warnings.
        # Detect sim-time reset (Gazebo restart/world reset)
        if (self.last_tf_stamp_sec > 1.0 and
                stamp_sec < self.last_tf_stamp_sec - 5.0):
            self.get_logger().warn(
                f'Sim-time reset detected ({stamp_sec:.3f}s vs '
                f'last {self.last_tf_stamp_sec:.3f}s) — resetting TF guard')
            self.last_tf_stamp_sec = 0.0

        if stamp_sec > self.last_tf_stamp_sec:
