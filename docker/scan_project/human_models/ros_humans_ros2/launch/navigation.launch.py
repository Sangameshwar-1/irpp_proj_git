...
def _launch_setup(context, *args, **kwargs):
    """Resolve all LaunchConfiguration values at launch time so gz_args
    is a clean Python string (not a fragile list of Substitutions)."""

    world_file      = context.launch_configurations["world"]
    goal_x          = context.launch_configurations["default_goal_x"]
    goal_y          = context.launch_configurations["default_goal_y"]
    ...
    # Conditions: camera vs GT  (plain Python, fully resolved)
    is_camera_str = (perception_mode == "camera")
    is_gt_str     = (perception_mode != "camera")

    # Gazebo args: headless = server-only + headless rendering (sensors work, no GUI)
    gz_prefix = "-s --headless-rendering " if headless == "true" else ""
    ...
    return [
        # ═══════════════════════════════════════════════════════════════
        #  SIMULATION  — gz_args is now a single resolved f-string
        # ═══════════════════════════════════════════════════════════════

        IncludeLaunchDescription(
            PythonLaunchDescriptionSource(gazebo_launch),
            launch_arguments={"gz_args": f"{gz_prefix}-r {world_file}"}.items()),
        ...
        # ═══════════════════════════════════════════════════════════════
        #  TF + POINT CLOUDS  (provides TF tree for RViz)
        # ═══════════════════════════════════════════════════════════════
        ...
        Node(package="ros_humans_ros2", executable="move_humans",
             ...
             parameters=[{"rate": 10.0,
                           ...
                           "use_sim_time": True}],
             condition=IfCondition(str(is_gt_str).lower())),
        ...
        Node(package="ros_humans_ros2", executable="move_humans",
             ...
             parameters=[{"rate": 10.0,
                           ...
                           "use_sim_time": True}],
             condition=IfCondition(str(is_camera_str).lower())),
        ...
        # ═══════════════════════════════════════════════════════════════
        #  NAVIGATION + VISUALIZATION NODES
        # ═══════════════════════════════════════════════════════════════
        ...
        Node(package="ros_humans_ros2", executable="global_planner",
             ...
             parameters=[{"planner_resolution": 0.2,
                           ...
                           "auto_start": True,
                           "default_goal_x": float(goal_x),
                           "default_goal_y": float(goal_y),
                           "room_min_x": -13.0,
                           "room_max_x": 13.0,
                           "room_min_y": -13.0,
                           "room_max_y": 13.0,
...
def generate_launch_description():
    ...
    return LaunchDescription([
        ...
        DeclareLaunchArgument(
            "headless",
            default_value="false",
            description="Run Gazebo in headless server mode (no GUI window, "
                        "sensors still render via EGL). Use 'true' for Docker."),
